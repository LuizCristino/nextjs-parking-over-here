...
export function generate(
  ...
  const leftSide =
    typeof sides === 'number' ? sides : sides.left ?? defaultOptions.sides;
  const rightSide =
    typeof sides === 'number' ? sides : sides.right ?? defaultOptions.sides;

  const leftSkipStr =
    typeof skipString === 'string'
      ? skipString
      : skipString.left ?? defaultOptions.skipString;
  const rightSkipStr =
    typeof skipString === 'string'
      ? skipString
      : skipString.right ?? defaultOptions.skipString;
  ...
  if (start <= end) {
    ...
    if (end - +pagination[pagination.length - 1] === 1) {
      ...
      rightSide > 1 &&
      end - +pagination[pagination.length - 1] === 2
    ) {
